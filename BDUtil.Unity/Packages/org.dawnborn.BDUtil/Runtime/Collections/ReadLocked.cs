...
namespace BDUtil
{
    ...
    [Serializable]
    public class ReadLockedSet<T> : Raw.ReadLocked<T, Set<T>, IReadOnlyCollection<T>, IEnumerable<T>>
    {
        [SerializeField] Set<T> _Data;
        protected override Set<T> Data { get => _Data; set => _Data = value; }
    }
    [Serializable]
    public class ReadLockedOrderedDeque<T> : Raw.ReadLocked<T, OrderedDeque<T>, IReadOnlyDeque<T>, IEnumerable<T>>
    {
